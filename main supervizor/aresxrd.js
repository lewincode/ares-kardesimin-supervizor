...
fs.readdir('./src/commands/', (err, files) => {
  ...
  console.log(`[ARESXRD] Toplamda ${files.length} Adet Komut Yüklenecek.`);
  files.forEach(f => {
    fs.readdir("./src/commands/" + f, (err2, files2) => {
      files2.forEach(file => {
        let props = require(`./src/commands/${f}/` + file);
        console.log(`[ARESXRD] ${props.conf.name} İsimli Komut Yüklendi!`);
...
client
  .login(conf.Main.token)  // Glitchde Çalışması İçin process.env.token
  .then(() => console.log(`[ARESXRD] Bot Başarıyla ${client.user.tag} Olarak Giriş Yaptı!`))
  ...
  fs.readdirSync('./src/Slashcommands/').forEach(async category => {
	...
	const rest = new REST({ version: '9' }).setToken(conf.Main.token);
  (async () => {
	try {
		console.log('[ARESXRD] Slash ve Komutlar yükleniyor.');
		await rest.put(
			Routes.applicationGuildCommands(conf.Main.BotClientID, conf.GuildID),
			{ body: slashcommands },
		).then(() => {
			console.log('[ARESXRD] Slash ve Context Komutlar yüklendi.');
		});
	}
	catch (e) {
		console.error(e);
